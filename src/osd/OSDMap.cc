				     ...
				     int *primary) const
{
  ...
  //
  // track a 'fallback' choice that we rejected probabilistically in
  // case we don't pick anyone better.
  int fallback_pos = -1;
  int pos = -1;
  for (unsigned i = 0; i < osds->size(); ++i) {
    int o = (*osds)[i];
	...
	(crush_hash32_2(CRUSH_HASH_RJENKINS1,
			seed, o) >> 16) >= a) {
      // we chose not to use this primary.  note it as a fallback
      // in case we don't pick anyone else.
      if (fallback_pos < 0)
	fallback_pos = i;
    } else {
      pos = i;
  ...
  if (pos < 0)
    pos = fallback_pos; // we didn't pick anyone

  if (pos < 0)
    return;             // we didn't skip anyone either :(
